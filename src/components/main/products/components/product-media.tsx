...
import { UploadProductImage } from "./dialog/upload-product-image";
import {
	DndContext,
	closestCenter,
	KeyboardSensor,
...
import { updateProductImagesOrderAPI } from "@/api/ecommerce";
...
export const ProductMedia = ({
	...
	productId: number;
	images: ProductImage[];
	deleteImages: (productId: string, imageKey: string) => void;
	...
	const handleSaveOrder = async () => {
		await requestHandler(
			() => updateProductImagesOrderAPI(productId, localImages),
			null,
			async () => {
				await fetchProduct();
				setIsReordering(false);
				toast.success("Product image order updated successfully.");
			},
			(e) => {
				toast.error(e);
			}
						...
						>
							{localImages.map((image) => (
								<SortableImage
									...
									image={image}
									onDelete={() => {
										deleteImages(productId.toString(), image.key);
									}}
								/>
							))}
						</SortableContext>
